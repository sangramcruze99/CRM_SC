    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white/[0.04] p-4 rounded-3xl border border-white/[0.08] backdrop-blur-xl">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto">
          ...
          <button
            ...
            className="px-3.5 py-1.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-slate-950 font-bold rounded-xl text-xs flex items-center gap-1.5 shadow-md ml-auto cursor-pointer"
          >
            <UserPlus size={13} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        {filtered.map((emp) => (
          <div
            key={emp.id}
            className="bg-white/[0.04] backdrop-blur-2xl border border-white/[0.08] rounded-3xl overflow-hidden hover:border-emerald-500/40 hover:shadow-xl transition-all shadow-[0_8px_32px_0_rgba(0,0,0,0.37)] group flex flex-col justify-between"
          >
            <div className="p-6 flex flex-col items-center text-center border-b border-white/[0.06]">
              <div className="relative mb-3">
                <img
                  src={emp.avatar}
                  alt={emp.firstName}
                  className="w-16 h-16 rounded-2xl object-cover border border-white/10 shadow-lg shadow-emerald-500/20 group-hover:scale-105 transition-transform"
                />
                <span className="absolute -bottom-2 -right-2 px-2 py-0.5 rounded-full text-[9px] font-mono font-bold bg-slate-950 border border-amber-500/50 text-emerald-300">
                  L{emp.level}
                </span>
              </div>
              ...
              <p className="text-xs text-emerald-400 font-semibold mt-0.5 line-clamp-1">{emp.jobTitle}</p>
