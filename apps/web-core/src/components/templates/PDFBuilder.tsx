    ...
    <div className="h-full flex flex-col max-w-7xl mx-auto text-white space-y-6">
      ...
      <div className="bg-white/[0.04] backdrop-blur-2xl border border-white/[0.08] rounded-3xl p-5 px-6 flex items-center justify-between shadow-[0_8px_32px_0_rgba(0,0,0,0.37)]">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="px-4 py-2 bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 hover:from-amber-400 hover:to-orange-400 text-slate-950 text-xs font-bold rounded-xl shadow-lg shadow-orange-500/25 transition-all flex items-center gap-2 cursor-pointer">
      ...
      <div className="flex-1 flex gap-6 overflow-hidden">
        ...
        <div className="w-80 bg-white/[0.04] backdrop-blur-2xl border border-white/[0.08] rounded-3xl p-6 overflow-y-auto shadow-[0_8px_32px_0_rgba(0,0,0,0.37)]">
          <div className="flex items-center gap-2 mb-6 text-white font-bold text-sm">
            <Settings size={18} className="text-amber-400" /> Template Properties
          </div>
          ...
          <div className="space-y-5">
            ...
            <div className="pt-4 border-t border-white/[0.08]">
               ...
               <div className="flex flex-wrap gap-2">
                 <span className="px-2.5 py-1 bg-amber-500/15 border border-amber-500/30 text-amber-300 rounded-lg text-xs font-mono cursor-pointer hover:bg-amber-500/25">{`{{record.id}}`}</span>
                 <span className="px-2.5 py-1 bg-amber-500/15 border border-amber-500/30 text-amber-300 rounded-lg text-xs font-mono cursor-pointer hover:bg-amber-500/25">{`{{record.createdAt}}`}</span>
                 <span className="px-2.5 py-1 bg-amber-500/15 border border-amber-500/30 text-amber-300 rounded-lg text-xs font-mono cursor-pointer hover:bg-amber-500/25">{`{{tenant.name}}`}</span>
               </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 bg-white/[0.02] border border-white/[0.08] rounded-3xl p-8 flex flex-col items-center overflow-y-auto">
           <div className="w-full max-w-3xl flex flex-col gap-4">
             <div className="flex items-center justify-between">
               <h3 className="text-xs font-bold text-slate-400 flex items-center gap-2 uppercase tracking-wider"><Printer size={16} className="text-amber-400" /> Live HTML Preview (A4)</h3>
             </div>
             {/* A4 Paper Mockup */}
             <div className="bg-slate-900 shadow-2xl aspect-[1/1.414] w-full p-12 flex flex-col text-white border border-white/[0.1] rounded-2xl">
                <h1 className="text-2xl font-bold uppercase text-center mb-8 border-b-2 border-white/20 pb-4 text-amber-400">{title}</h1>
