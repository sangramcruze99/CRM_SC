...
export async function POST(req: NextRequest) {
  try {
    ...
    if (lower.includes('deal') || lower.includes('pipeline') || lower.includes('sales') || lower.includes('revenue') || lower.includes('opportunity')) {
      ...
      const topDeal = activeDeals[0] || { title: 'Enterprise Expansion Deal', amount: 48000, stage: 'PROPOSAL' };
    ...
    if (lower.includes('invoice') || lower.includes('unpaid') || lower.includes('payment') || lower.includes('overdue') || lower.includes('collect') || lower.includes('finance')) {
      ...
      const overdueList = Array.isArray(invoices) ? invoices.filter((i) => i.status === 'OVERDUE' || i.status === 'UNPAID') : [];

      const topInv = overdueList[0] || { invoiceNumber: 'INV-2026-089', amount: 6200, dueDate: '2026-09-01' };

      return NextResponse.json({
        department: 'Finance AI (Midas)',
        departmentKey: 'finance',
        intent: 'INVOICES_COLLECTION',
        answer: `I analyzed your accounts receivable. There are ${overdueList.length} outstanding invoices. The primary invoice needing attention is #${topInv.invoiceNumber || '1042'} for $${Number(topInv.amount || 0).toLocaleString()}. I can send a polite payment reminder.`,
        suggestedActions: [
    ...
    if (lower.includes('churn') || lower.includes('customer') || lower.includes('risk') || lower.includes('unhappy') || lower.includes('satisfaction') || lower.includes('health')) {
      ...
      const contactSample = Array.isArray(contacts) && contacts.length > 0 ? contacts[0] : { firstName: 'Sarah', lastName: 'Connor', email: 'sconnor@acme.corp' };

      return NextResponse.json({
        department: 'Customer Success AI (Athena)',
        departmentKey: 'cs',
        intent: 'CHURN_PREVENTION',
        answer: `I scanned account engagement across your customer base. Overall health is stable, but account "${contactSample.firstName} ${contactSample.lastName}" has experienced declining usage over the last 14 days and may benefit from a check-in.`,
        suggestedActions: [
          ...
          { label: 'Create high-priority retention task', action: 'CREATE_TASK', taskTitle: `Customer Health Audit: ${contactSample.firstName}` }
        ],
        dataSummary: {
          accountsScanned: Array.isArray(contacts) ? contacts.length : 12,
          atRiskCount: 1,
        },
      });
    }
