...
const initialTables: DiningTable[] = [
  { id: 'T-01', tableNumber: 'Table 1', capacity: 2, status: 'AVAILABLE', currentBill: 0, server: 'Marco' },
  { id: 'T-02', tableNumber: 'Table 2', capacity: 4, status: 'AVAILABLE', currentBill: 0, server: 'Elena' },
  { id: 'T-03', tableNumber: 'Table 3', capacity: 6, status: 'AVAILABLE', currentBill: 0, server: 'Elena' },
  { id: 'T-04', tableNumber: 'Booth 1', capacity: 4, status: 'AVAILABLE', currentBill: 0, server: 'Marco' },
  { id: 'T-05', tableNumber: 'Booth 2', capacity: 4, status: 'AVAILABLE', currentBill: 0, server: 'Leo' },
    ...
    <div className="space-y-6 max-w-7xl mx-auto text-white">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="bg-white/[0.04] backdrop-blur-2xl border border-white/[0.08] rounded-3xl p-5 shadow-[0_8px_32px_0_rgba(0,0,0,0.37)]">
          ...
          <div className="text-xs text-emerald-400 mt-2 font-bold">{Math.round((occupiedCount / tables.length) * 100)}% Capacity Seated</div>
        </div>
                  ...
                  <div
                    ...
                    className={`p-4 rounded-2xl border transition-all flex flex-col justify-between space-y-3 ${
                      t.status === 'AVAILABLE'
                        ...
                        : t.status === 'OCCUPIED'
                        ? 'border-emerald-500/30 bg-emerald-500/10 hover:border-amber-400/60'
                        : t.status === 'BILLING'
            ...
            <div className="space-y-3">
              {kitchenOrders.map((kot) => (
                ...
                >
                  <div className="flex items-center justify-between border-b border-white/[0.06] pb-2">
                    <div className="flex items-center gap-2">
                      <span className="font-mono font-bold text-xs text-emerald-300 bg-emerald-500/20 px-2 py-0.5 rounded border border-emerald-500/40">
                        {kot.id}
                      </span>
                  ...
                  {kot.specialInstructions && (
                    <div className="p-2 bg-emerald-500/10 rounded-xl border border-emerald-500/30 text-[10px] font-semibold text-emerald-300">
              ...
              {kitchenOrders.length === 0 && (
                <div className="py-12 text-center text-slate-500 text-xs font-medium">
                  No active tickets in kitchen display queue. Orders placed from dining tables or online menus will appear here.
                </div>
              )}
            </div>
          </div>
