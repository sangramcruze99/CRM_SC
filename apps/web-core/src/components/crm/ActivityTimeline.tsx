...
export function ActivityTimeline({ entityType, entityId }: { entityType: 'contact' | 'deal' | 'company', entityId: string }) {
  ...
  return (
    <div className="space-y-5 text-white">
      <div className="bg-white/[0.04] backdrop-blur-2xl border border-white/[0.08] rounded-3xl p-4 shadow-sm">
        <div className="space-y-3">
          <textarea 
            placeholder="Log an interaction note or action item..." 
            value={newNote}
            onChange={(e) => setNewNote(e.target.value)}
            className="w-full bg-white/[0.05] border border-white/[0.1] rounded-2xl px-3.5 py-2.5 text-xs text-white focus:outline-none focus:bg-white/[0.08] min-h-[80px] font-medium placeholder-slate-400"
          />
      ...
      <div className="space-y-3.5">
        {loading ? (
          <p className="text-xs text-slate-400 text-center py-4 font-medium">Loading timeline events...</p>
        ) : activities.length === 0 ? (
            ...
            <div key={activity.id} className="bg-white/[0.04] backdrop-blur-2xl border border-white/[0.08] rounded-3xl overflow-hidden shadow-xs hover:border-amber-500/40 transition-colors">
              <div className="px-4 py-2.5 border-b border-white/[0.08] flex flex-row items-center space-x-3 bg-white/[0.02]">
                ...
                <div className="flex-1">
                  <h3 className="text-xs font-bold text-white">{activity.type}</h3>
              ...
              <div className="p-4">
                <p className="text-xs text-slate-300 font-medium whitespace-pre-wrap leading-relaxed">{activity.content}</p>
              </div>
            </div>
          ))
        )}
