...
export function MobileAppDock() {
  ...
  return (
    <div className="md:hidden fixed bottom-3 inset-x-0 z-40 flex justify-center px-4 pointer-events-none">
      <div className="flex items-center gap-1.5 p-2 bg-[#0c1411]/90 border border-white/15 rounded-full shadow-[0_12px_40px_rgba(0,0,0,0.8)] backdrop-blur-2xl pointer-events-auto">
        {dockItems.map((item) => {
          const isActive =
            pathname === item.href ||
        ...
        <button
          ...
          className="flex items-center justify-center w-11 h-11 rounded-full bg-white/10 hover:bg-white/20 text-emerald-400 border border-emerald-400/30 transition-all active:scale-95 cursor-pointer shadow-xs"
