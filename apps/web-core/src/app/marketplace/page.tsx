    ...
    <div className="h-full flex flex-col space-y-6 max-w-7xl mx-auto text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 overflow-y-auto pb-6">
        {plugins.map((plugin: any) => (
          <div key={plugin.id} className="bg-white/[0.04] backdrop-blur-2xl border border-white/[0.08] rounded-3xl p-6 flex flex-col hover:border-emerald-500/40 hover:shadow-xl transition-all shadow-[0_8px_32px_0_rgba(0,0,0,0.37)] group">
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center space-x-3">
                <div className="w-11 h-11 rounded-2xl bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 flex items-center justify-center shadow-2xs">
                  <Package size={22} />
                </div>
                <div>
                  <h3 className="font-bold text-white text-base group-hover:text-emerald-300 transition-colors">{plugin.name}</h3>
            ...
            <div className="flex items-center justify-between mt-auto pt-4 border-t border-white/[0.06]">
              <div className="flex items-center space-x-1.5 text-xs text-slate-400">
                <Star size={13} className="text-emerald-400 fill-amber-400" />
              ...
              {plugin.isInstalled ? (
                ...
                <button className="flex items-center space-x-1.5 px-3.5 py-1.5 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-amber-400 hover:to-orange-400 text-slate-950 rounded-xl text-xs font-bold transition-all shadow-md shadow-emerald-500/20 active:scale-[0.98] cursor-pointer">
                  <Download size={13} />
